...
if __name__ == "__main__":
    ...
    for post in posts.json()['posts']:
        if post['tags'] and any(tag['slug'] == "melmelboo"
                                for tag in post['tags']):
            ghost_cur.execute("SELECT COUNT(*) FROM posts WHERE title = ?",
                              (post['title'], ))
            res = ghost_cur.fetchone()
            if not res[0]:
                art_uuid = str(uuid.uuid4())
                ghost_cur.execute("""
                                  INSERT INTO posts(uuid, title, slug,
                                                    markdown, html, image,
                                                    status, language,
                                                    author_id,
